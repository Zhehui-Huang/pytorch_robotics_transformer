...
class TransformerNetwork(nn.Module):
    ...
    def __init__(
            self,
            input_tensor_space: spaces.Dict,  # observation space like dict. keys are image, natural_language_embedding
            output_tensor_space: spaces.Dict,
            # action space like dict.
            # keys are world_vector, rotation_delta, gripper_closeness_action, terminate_episode
            train_step_counter: int = 0,
            vocab_size: int = 256,
            # Dimensionality of tokens from the output layer.
            # This is also dimensionality of tokens from the input layer.
            token_embedding_size: int = 512,
            # RT1ImageTokenizer outputs(=context_image_tokens) has embedding dimension of token_embedding_size.
            # This will finally be utilized in 1x1 Conv in EfficientNetEncoder class.
            num_layers: int = 1,
            layer_size: int = 4096,
        ...
        self._state_space = spaces.Dict(
            {
                ...
                # Our data:
                # context_image_tokens + action_tokens + context_image_tokens + action_tokens + context_image_tokens ...
                # 1 time step means [context_image_tokens + action_tokens]
                # seq_idx means which time steps we are.
                # But it is adjusted to time_sequence_length when it exceeds time_sequence_length.
            }
        )
    ...
    # _default_attention_mask is modified causal mask because
    # we will only use observations tokens when predicting actions.
    ...
    def forward(self,
                observations: Dict[str, torch.Tensor], network_state: Dict[str, torch.Tensor],
                # network_state retain observation tokens, action tokens, seq_idx.
                ):
        ...
        Returns:
            A tuple `(Detokenize output actions, network state)`.
        """
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            start_index = (
                    transformer_shift + self._tokens_per_context_image + action_t * self._single_time_step_num_tokens)
            ...
            action_logits_for_training = action_logits.view(b, t, self._tokens_per_action,
                                                            -1)  # (bs, t, self._tokens_per_action, vocab_size)
            ...
            action_loss = torch.mean(
                self._loss_object(action_logits_for_training.permute(0, 3, 1, 2), action_tokens) / num_items,
    ...
    @staticmethod
    def _get_batch_size_and_seq_len(network_state):
    ...
    def _transformer_call(
        ...
        input_token_sequence = self._assemble_input_token_sequence(context_image_tokens, action_tokens,
                                                                   batch_size)  # [b, t*num_tokens, emb_dim]
        # run transformer
        output_tokens, self._attention_scores = self._transformer(input_token_sequence,
                                                                  attention_mask)  # (bs, t*num_tokens, vocab_size)
    ...
    def _assemble_input_token_sequence(self, context_image_tokens, action_tokens, batch_size):
        ...
        # This removes autoregressively conditioning on actions becuase it did not benefit performance and slowed
        # inference.
        ...
        input_token_sequence = input_token_sequence.view(batch_size, -1,
                                                         self._token_embedding_size)  # [b, t*num_tokens, emb_dim]
    ...
    def _tokenize_images(self, observations, network_state):
        ...
        # update network state at inference we retain some context_image_tokens to accelerate computation. At
        # inference, context_image_tokens : (batch, 1, num_tokens, embedding_dim) At inference, network_state stores
        # context_image_tokens of pastime steps. Here, we combine past context_image_tokens of network_state with
        # current context_image_tokens. network_state only store tokens within time_sequence_length time steps. This
        # means network_state does not store the tokens for all past steps, but only for time_sequence_length time
        # steps. if current time step >= time_sequence_length, we store context_image_tokens after we discard the
        # oldest context_image_tokens. Here, we implement that by shifting state_image_token to the left.
        if outer_rank == 1:  # This is an inference call
            state_image_tokens = network_state[
                'context_image_tokens']  # (1, time_sequence_length, tokens_per_context_image, token_embedding_size)
            ...
            # if seq_idx == time_sequence_length, state_image_tokens will be shifted to the left a long time axis
            # seq_idx will be incremented in forward function. But it is adjusted
            # so that it never exceed time_sequence_length.
            # Therefore, shifting will always occur when time step exceeds time_sequence_length.
    ...
    def _tokenize_actions(self, observations, network_state):
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            if self._actions is None:
                ...
                b, t = self._get_batch_size_and_seq_len(network_state)
                action_tokens = torch.zeros(size=(b, t, self._tokens_per_action), dtype=torch.int32)
            else:
